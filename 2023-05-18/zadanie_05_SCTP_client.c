...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        sctp_sendmsg(sfd, (void*) buf, (size_t) strlen(buf) + 1, NULL, 0, 0, 0, 0, 0, 0);
        memset(&raddr, 0, sizeof(raddr));
        memset(&buf, 0, sizeof(buf));
        sl = sizeof(raddr);
        cfd = accept(sfd, (struct sockaddr*) &raddr, &sl);
        flags=0;
        no = sctp_recvmsg(cfd, (void*)buf, sizeof(buf), (struct sockaddr*) NULL, 0, &sndrcvinfo, &flags);
